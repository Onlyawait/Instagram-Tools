...
Module Module1
    ...
    Sub Main()
        Console.Title = "Unlike Posts by await | Insta | @_824 "
    ...
    Async Function GetPostId() As Task
        ...
        Dim baseAddress = New Uri("https://i.instagram.com/api/v1/feed/liked/")
        ...
        Using HttpClient As New HttpClient(Handler)
            ...
            If Respon.Contains("id") Then
                ...
                Await Unlike_Post(PostId, UsernamePost)
            ElseIf Not Respon.Contains("id") Then
                Stop1 = True
                Write("Unlike ALL Post", "Enter To Closed", ConsoleColor.Green, True)
            Else
                Write("Error", $"@{user}", ConsoleColor.Red)
            End If
        End Using
    End Function
    Async Function Unlike_Post(PostId As String, UsernamePost As String) As Task
        Dim Handler As New HttpClientHandler()
        Dim baseAddress = New Uri($"https://i.instagram.com/api/v1/media/{PostId}/unlike/")
        Handler.CookieContainer = Cookie
        Handler.UseCookies = True
        Using HttpClient As New HttpClient(Handler)
            HttpClient.BaseAddress = baseAddress
            ...
            If Respon.Contains("ok") Then
                Count += 1
                Write("Unliked", "@" + UsernamePost, ConsoleColor.Green, True, Count)
            Else
